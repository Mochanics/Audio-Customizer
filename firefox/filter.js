...
//Context array - stores all audio contexes for the page
let contexes = [];
...
async function filter(instance) { //Filter creation function
	...
	const context = new AudioContext(); //Creates a new audio context
	contexes.push(context); //Adds it to the array
...
//Closes and recreates the audio contexes when the user changes a plugin setting from the settings popup
function reset() {
	for (let i = 0; i < contexes.length; i++) {
		contexes[i].close()
	}
